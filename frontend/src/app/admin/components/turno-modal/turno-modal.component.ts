          ...
          <div class="space-y-2">
            ...
            <ng-template #buscador>
              ...
              <div class="border border-stone-200 rounded-xl overflow-hidden divide-y divide-stone-100 max-h-40 overflow-y-auto">
                ...
                <p *ngIf="pacientesFiltrados().length === 0" class="px-4 py-3 text-xs text-stone-400 italic">
                  No se encontraron pacientes. Por ahora solo se pueden agendar turnos a pacientes existentes.
                </p>
              </div>
            </ng-template>
          </div>
              ...
              <div class="space-y-1">
                ...
                <div class="relative">
                  <select [ngModel]="servicioNombre()" (ngModelChange)="servicioNombre.set($event)"
                    ...
                    <option *ngFor="let s of adminService.services()" [value]="s.name">{{ s.name }} ({{ s.durationMinutes }} min)</option>
                  </select>
...
export class TurnoModalComponent {
  ...
  constructor() {
    effect(() => {
      ...
      const servicios = this.adminService.services();
