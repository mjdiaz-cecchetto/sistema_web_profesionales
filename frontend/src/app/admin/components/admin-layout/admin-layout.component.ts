      ...
      <aside class="hidden md:flex flex-col w-[264px] bg-white border-r border-stone-200 shrink-0 h-full overflow-y-auto">
        ...
        <div class="p-4 shrink-0 border-t border-stone-100">
          <div class="bg-stone-50 border border-stone-200 rounded-2xl p-3.5 flex items-center gap-3">
            <div class="w-10 h-10 rounded-full overflow-hidden ring-2 ring-teal-200 shrink-0 bg-stone-100">
            ...
            <a routerLink="/client" target="_blank" title="Ver página pública"
        ...
        <header class="h-16 md:h-20 bg-white border-b border-stone-200 px-4 md:px-6 flex justify-between items-center relative z-30 shrink-0">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              ...
              <p class="hidden sm:block text-[11px] text-stone-400 font-medium mt-1">Gestioná tu consultorio desde un solo lugar</p>
            </div>
          </div>

          <div class="flex items-center gap-2 md:gap-3 shrink-0">
            <a routerLink="/client" target="_blank" class="hidden sm:inline-flex btn-secondary !py-2 !px-4 !text-xs items-center gap-2">
...
export class AdminLayoutComponent {
  ...
  nombre = computed(() => this.adminService.profile()?.nombre ?? 'Cargando…');
  titulo = computed(() => this.adminService.profile()?.titulo ?? '');
  avatar = computed(() => this.adminService.profile()?.avatarUrl || 'dra-elena.jpg');
