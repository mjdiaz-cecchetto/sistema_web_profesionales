      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-2 shrink-0">
        ...
        <div class="flex items-center gap-2">
          ...
          <button (click)="filtrosAbiertos.set(!filtrosAbiertos())"
                  [ngClass]="filtrosAbiertos() || filtrosAvanzadosCount() > 0 ? 'bg-teal-100 text-teal-900 border-teal-300' : 'bg-white text-stone-500 border-stone-200 hover:text-teal-800 hover:border-teal-300'"
            ...
            Filtros
            <span *ngIf="filtrosAvanzadosCount() > 0" class="bg-white text-teal-900 border border-teal-300 px-1.5 py-0.5 rounded-full text-[9px]">{{ filtrosAvanzadosCount() }}</span>
          </button>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
          ...
          <div class="space-y-1.5 sm:col-span-2 lg:col-span-1">
            ...
            <div class="flex gap-1">
              <button *ngFor="let r of rangosRapidos"
                      (click)="setRango(r.value)"
                      [ngClass]="rangoActivo() === r.value
          ...
          <app-date-picker label="Fecha Desde" placeholder="Desde..."
            [value]="dateFromFilter()" (valueChange)="onDateFromChange($event)">
          </app-date-picker>

          <app-date-picker label="Fecha Hasta" placeholder="Hasta..."
            [value]="dateToFilter()" (valueChange)="onDateToChange($event)">
          </app-date-picker>

          <div class="space-y-1.5">
            <label class="field-label">Servicio</label>
            <div class="relative">
              <select [ngModel]="serviceFilter()" (ngModelChange)="onServiceChange($event)"
                      class="input !py-2 !text-xs appearance-none cursor-pointer !pr-8">
        ...
        <div class="flex justify-between items-center gap-3 flex-wrap">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="relative w-48">
              ...
              <svg class="w-4 h-4 text-stone-400 absolute right-3 top-2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
            </div>
          </div>

          <button *ngIf="filtrosActivosCount() > 0" (click)="resetFilters()"
            ...
            Limpiar {{ filtrosActivosCount() }} {{ filtrosActivosCount() === 1 ? 'filtro' : 'filtros' }}
          </button>
        </div>
      </div>
      ...
      <div class="card flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div class="flex-1 min-h-0 overflow-y-auto max-h-[60vh] xl:max-h-none">
          <div *ngIf="filteredAppointments().length > 0; else noResults" class="divide-y divide-stone-100">
            <div *ngFor="let appt of paginatedAppointments()"
              ...
              <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-y-2.5 gap-x-4 items-center">
                ...
                <div class="lg:col-span-4 space-y-1">
                  <div class="flex items-center gap-2 flex-wrap">
                    <h4 class="font-extrabold text-stone-800 text-sm truncate" [title]="appt.patientName">{{ appt.patientName }}</h4>
                ...
                <div class="lg:col-span-4 text-[11px] text-stone-500 space-y-0.5">
                  <p class="truncate"><span class="font-semibold text-stone-400">Tel:</span> {{ appt.patientPhone }} <span class="mx-1 hidden lg:inline">•</span> <br class="lg:hidden"/><span class="font-semibold text-stone-400">Email:</span> {{ appt.patientEmail }}</p>
                  <p class="truncate"><span class="font-semibold text-stone-400">DNI:</span> {{ appt.patientDni }} <span class="mx-1 hidden lg:inline">•</span> <br class="lg:hidden"/><span class="font-semibold text-stone-400">Cob.:</span>&nbsp;<span class="font-semibold text-stone-600">{{ appt.healthInsurance }}</span></p>
                </div>
          ...
          <ng-template #noResults>
            <div class="py-14 text-center space-y-3">
              <div class="w-14 h-14 mx-auto rounded-2xl bg-stone-100 flex items-center justify-center">
                <svg class="w-7 h-7 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
              </div>
              <p class="text-sm text-stone-500">No se encontraron turnos con los filtros aplicados.</p>
              <button (click)="resetFilters()" class="text-xs text-teal-700 font-bold hover:underline">Restablecer filtros</button>
            </div>
          </ng-template>
        </div>
